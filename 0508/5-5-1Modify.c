// PROGRAM	: 5-5-1Modify.c										2018.0505
// FUNCTION	: LCD Control with Time/Alarm  Function				By Steven
//            ALARM Made By CTM Compare Match Output TP0_1[PC5]
#include "HT66F70A.h"
#include "5-5-1.H"
#include "MyType.H"
...
volatile u8 Second;
volatile _Bool	FG_HalfSec,FG_Second,FG_Adj,FG_Alarm,FG_Select,FG_State,FG_OK;
struct TimeFormat
{	u8 Hour;
	u8 Minute;
}	Time,Alarm;
...
{	u8 i;
	...
	_scomc=0b01111111; 
	_mf0e=1; _t2ae=1;
	_tm0c0=0; _tm0c1=0b00110001; _pcs2=0x20;				//PC5 as TP0
	...
	{	if(!(SW_Adj | SW_Enter))					   		//Both Keys Pressed ?
		...
		{ 	while(FG_OK)
			{	_tm0al=(u8)500; _tm0ah=500>>8;
				_t0on=1; PAUSE(10);_t0on=0;
				_tm0al=(u8)1000; _tm0ah=1000>>8;
				_t0on=1; PAUSE(10);_t0on=0;
				if(!(SW_Adj & SW_Enter)) FG_OK=0;
			}
//			PAUSE(100);	
		}
		else	FG_OK=1;
